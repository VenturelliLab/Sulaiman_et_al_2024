...
def process_df(df, species):
    ...
    for treatment, comm_data in df.groupby("Treatments"):
        ...
        for i in range(len(t_eval)-1):
            data.append([treatment+f"_t{i}", t_eval[i:i+2], Y_measured[i:i+2]])
...
class gLV:
    def __init__(self, species, dataframe, verbose=True):
        ...
        self.adjoint = jit(jacfwd(lambda xt, yt, B: jnp.einsum("i,ij,j", yt-xt, B, yt-xt)/2.))
        self.runODEA = jit(lambda t_eval, xt, at, params: runODEA(t_eval, xt, at, params, self.dXA_dt))
    ...
    def fit(self, evidence_tol=1e-3, nlp_tol=None, alpha_0=1e-3, patience=1, max_fails=2, beta=1e-3):
        ...
        while passes < patience and fails < max_fails:
            ...
            # self.Ainv = self.make_pos_def(self.Ainv, jnp.ones(self.n_params))
    ...
    def jacobian_adj(self, params):
        ...
        for treatment, t_eval, Y_measured in self.dataset:
            ...
            at = self.adjoint(output[-1], Y_measured[-1], self.Beta)

            # gradient of NLP
            grad_NLP += self.runODEA(t_eval, output[-1], at, params)
    ...
    def predict(self, x_test, t_eval, n_std=1., log=False):
        ...
        stdv = jnp.sqrt(vmap(jnp.diag)(COV))
