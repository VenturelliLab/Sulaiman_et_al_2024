...
def runODE(t_eval, x0, params, ctrl_params, dX_dt):
    # solve ODE model
    y = odeint(dX_dt, x0, t_eval, params, ctrl_params)
    return y

# define function to integrate adjoint sensitivity equations backwards
def runODEA(t_eval, xt, at, params, ctrl_params, dXA_dt):
    ...
    x0, a0, l0 = odeint(dXA_dt, xal, t_eval, params, ctrl_params)

    # return parameter gradient
    return l0[-1]

# define function that returns model sensitivity vector
def runODEZ(t_eval, x0, params, ctrl_params, dXZ_dt):
    # check dimensions

    # set initial condition of sensitivity matrix
    xz = (x0, np.zeros([len(x0), len(params)]))

    # solve augmented ODE model
    y, Z = odeint(dXZ_dt, xz, t_eval, params, ctrl_params)
...
class gLV:
    def __init__(self, species, dataframe, verbose=True):
        ...
        def dXA_dt(xa, t, params, s_present):
            ...
            y_dot, vjpfun = jax.vjp(lambda x, params: self.dX_dt(x,t,params,s_present), x, params)
            vjps = vjpfun(a)
        ...
        def dXZ_dt(xZ, t, params, s_present):
            ...
            # compute derivatives
            dxdt  = self.dX_dt(x, t, params, s_present)
            ...
            Jx = jacfwd(self.dX_dt, 0)(x, t, params, s_present)

            # time derivative of parameter sensitivity
            JxZ = Jx@Z # JxV(Z)

            # compute gradient of model w.r.t. parameters
            Jp = jacfwd(self.dX_dt, 2)(x, t, params, s_present)
        ...
        self.runODE  = jit(lambda t_eval, x, params, s_present: runODE(t_eval, x[0], params, s_present, self.dX_dt))

        # jit compile function to integrate forward sensitivity equations
        self.runODEZ = jit(lambda t_eval, x, params, s_present: runODEZ(t_eval, x[0], params, s_present, self.dXZ_dt))
        ...
        self.runODEA = jit(lambda t, xt, at, params, s_present: runODEA(jnp.array([0., t]), xt, at, params, s_present, self.dXA_dt))
    ...
    def update_hypers(self, beta=1e-3):
        ...
        for treatment, t_eval, Y_measured, s_present in self.dataset:
            ...
            # for each output
            output, G = self.runODEZ(t_eval, Y_measured, self.params, s_present)

            # Determine SSE
            Y_error = output[1:] - Y_measured[1:]
            yCOV += self.yCOV_next(Y_error, G[1:], Ainv)
    ...
    def objective(self, params):
        ...
        for treatment, t_eval, Y_measured, s_present in self.dataset:
            ...
            # for each output
            output = np.nan_to_num(self.runODE(t_eval, Y_measured, params, s_present))
    ...
    def jacobian_adj(self, params):
        ...
        for treatment, t_eval, Y_measured, s_present in self.dataset:
            ...
            for t, out, a in zip(t_eval[1:], output[1:], at[1:]):
                grad_NLP += self.runODEA(t, out, a, params, s_present)
    ...
    def jacobian_fwd(self, params):
        ...
        for treatment, t_eval, Y_measured, s_present in self.dataset:
            ...
            # for each output
            output, G = self.runODEZ(t_eval, Y_measured, params, s_present)
    ...
    def predict_point(self, x_test, teval, log=False):

        # make predictions given initial conditions and evaluation times
        s_present = np.array(x_test > 0, int)
        if log:
            x_init = np.atleast_2d(x_test)
        else:
            x_init = np.atleast_2d(np.log(x_test+1e-8))
        Y_predicted = np.nan_to_num(self.runODE(teval, np.atleast_2d(np.log(x_test+1e-8)), self.params, s_present))
    ...
    def predict(self, x_test, t_eval, n_std=1., log=False):
        ...
        # if the initial condition is already log transformed
        if log:
            x_init = np.atleast_2d(x_test)
        else:
            x_init = np.atleast_2d(np.log(x_test+1e-8))
        ...
        # simulate
        s_present = np.array(x_init[0] != 0, int)
        output, G = self.runODEZ(t_eval, x_init, self.params, s_present)
        ...
        # mean
        M = np.einsum("ij,j->ij", np.exp(output), s_present)

        # determine confidence interval for species
        L = np.einsum("ij,j->ij", np.exp(output - stdv), s_present)
        U = np.einsum("ij,j->ij", np.exp(output + stdv), s_present)
    ...
    def design(self, df_design, N, batch_size=512, Ainv_q=None):
        ...
        Gs = {}
        exp_names = []
        for treatment, t_eval, Y_measured, s_present in design_space:
        ...
        while N_selected < N:
            ...
            for exp in exp_sorted:
                treatment, t_eval, Y_measured, s_present = design_space[exp]
    ...
    def design_opt(self, df_design, s_ind, N, Ainv_q=None):
        ...
        for treatment, t_eval, Y_measured, s_present in self.dataset:
            # run model using current parameters, output = [n_time, n_sys_vars]
            outcome = np.einsum("ij,j->ij", np.exp(Y_measured), s_present)
            s_vals.append(outcome[-1, s_ind])

        # need to compare to log of best previous observation
        treatment, t_eval, Y_measured, s_present = self.dataset[np.argmax(s_vals)]
        ...
        def utility_opt(pred, G_next, Ainv):
            ...
            # stdv = jnp.sqrt((self.BetaInv + self.GAinvG(G_next, Ainv))[-1, s_ind, s_ind])
            L = jnp.linalg.cholesky(self.BetaInv + self.GAinvG(G_next, Ainv))
        ...
        for treatment, t_eval, Y_measured, s_present in design_space:
            ...
            # output is log scale, but zeros are not in log scale
            # so replace zeros with small numbers
            output[output==0.] = -8.
